...
const TOKEN = "NDc3MTg1MTk5OTAyMjk0MDI2.DlODtw.Ijuyew362rajV_PGxa22_9nqsbA";
...
bot.on("ready", async() => {
	...
	/*/Avisos/*/
	bot.channels.get('477185649493934091').send("**~Recebi alguma atualização ou fui modificado em algum!~**");
	//bot.channels.get('477185649493934091').send("**~Donate Netshoes atualizadado verifique r!donates no canal #comandos ou será kickado!~**\n@here");
	//bot.channels.get('477185649493934091').send("**Aeh caralho recebi mais uma atuazalição. ♥**\n```Verifique minha versão: r!att```\n@here");
	//bot.channels.get('477185649493934091').send(":fire:**~Novidades~**:fire:\n\n//\n@here");
	//bot.channels.get('477185649493934091').send("**Checkers PagSeguro e Netshoes adicionado com sucesso. Verifique:** r!chks ♥\n@here");
    //bot.channels.get('477185649493934091').send("**Ligando aguarde alguns instantes, espero que tenha sentido minha falta...** ♥");
   	//bot.channels.get('477185649493934091').send("**Iniciando... Em seguida desligando...**");
    //bot.channels.get('477185649493934091').send("Por causa do meu tempo parado eu estou em manutenção!!");
    ...
    bot.user.setActivity(`Você | r!ajuda`, {type:"LISTENING"}); /*/ Muda a atividade do bot para "assitindo" /*/
...
bot.on('guildMemberAdd', member => { 
    ...
    var role = member.guild.roles.find('name', 'ratos') //AUTO-CARGOS
    member.addRole(role); 
    ...
    .setDescription("Bem vindo ao servidor! " + "<@" + member.user.id + ">" + ", ser fizer baguncinha vamos comer seu bumbum.")
    .setFooter("Novo membro") 
    .setTimestamp() 
    let entrada = member.guild.channels.find('name', 'bem-vindos')
    entrada.send(embed) && member.send('**Aeh mano(a) bem-vindo(a), qualquer dúvida chame o DualRoot, ele irar lhe ajudar assim que puder.**');
});
...
///////////////////////////////////
    bot.on('message', async message => { 
		...
		if(cmd === `${prefix}`){ // comando vazio
			...
			message.channel.send("Oi, coloca algum pra mim executar ainda não sei adivinhar. Para listar meus comandos r!ajuda");
		}
		...
		if(cmd === `${prefix}ajuda`){ // comando ajuda
		...
		if(cmd === `${prefix}quer?`){ // comando checkers
			...
			message.channel.send("Quero mais cc full no painel do @DualRoot#0171.");
		}
		if(cmd === `${prefix}chks`){ // comando checkers
			const embed = new Discord.RichEmbed()
			.setAuthor("Checkers Disponível")
			.setDescription("\n**Netshoes > r!chk-netshoes**\n\n**PagSeguro > r!chk-pagseguro**\n\n**Go4Gold > r!chk-gold [OFF]**\n\n**PeixeUrbano > r!chk-peixe [OFF]**\n\n**MercadoLivre > r!chk-ml [OFF]**");
		message.channel.send({embed});
		}
		...
		if(cmd === `${prefix}chk-pagseguro`){
			...
			.setColor("RANDOM")
			.setAuthor("Code by DualRoot/Ryan") /*/https://chk-netshoes-by-familydualroot.000webhostapp.com/ /*/
			.setDescription("PagSeguro [ON]\n\n**>** [**Click-me**](https://dualbot-chk-pagseguro.000webhostapp.com/)\n\n`Ajude manter ON fornecendo hospedagem!`");
			message.channel.send({embed});
		}
		...
		if(cmd === `${prefix}chk-ml`){
			
			const embed = new Discord.RichEmbed()
			.setColor("RANDOM")
			.setAuthor("Code by DualRoot/Ryan") /*/https://chk-netshoes-by-familydualroot.000webhostapp.com/ /*/
			.setDescription("MercadoLivre [OFF]\n\n>**>** [**Click-me**]()\n\n`Ajude manter ON fornecendo hospedagem!`");
			message.channel.send({embed});
		}
		...
		if(cmd === `${prefix}userinfo`){ // Melhorei o comando de userinfo
			...
			const embed = new Discord.RichEmbed()
				...
				.addField("Nome:", `${member.nickname !== null ? `${member.nickname}` : 'Nada'}`, true)
		...
		//NSFW ILIMITADO
		if(cmd === `${prefix}nsfw`){
    		if (!message.channel.nsfw) return message.reply("Este comando só pode ser usado no canal NSFW!");
